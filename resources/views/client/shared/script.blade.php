<!-- jQuery -->
{{-- <script src="{!! asset('bower_components/client-view/js/jquery.min.js') !!}"></script> --}}

<script src="{!! asset('client/js/jquery-3.3.1.min.js') !!}"></script>
<!-- jQuery Easing -->
<script src="{!! asset('bower_components/client-view/js/jquery.easing.1.3.js') !!}"></script>
<!-- Bootstrap -->
<script src="{!! asset('bower_components/client-view/js/bootstrap.min.js') !!}"></script>
<!-- Waypoints -->
<script src="{!! asset('bower_components/client-view/js/jquery.waypoints.min.js') !!}"></script>
<!-- Carousel -->
<script src="{!! asset('bower_components/client-view/js/owl.carousel.min.js') !!}"></script>
<!-- countTo -->
<script src="{!! asset('bower_components/client-view/js/jquery.countTo.js') !!}"></script>

<!-- Stellar Parallax -->
<script src="{!! asset('bower_components/client-view/js/jquery.stellar.min.js') !!}"></script>

<!-- Magnific Popup -->
<script src="{!! asset('bower_components/client-view/js/jquery.magnific-popup.min.js') !!}"></script>
<script src="{!! asset('bower_components/client-view/js/magnific-popup-options.js') !!}"></script>

<!-- Datepicker -->
<script src="{!! asset('bower_components/client-view/js/bootstrap-datepicker.min.js') !!}"></script>

<!-- Main -->
<script src="{!! asset('bower_components/client-view/js/main.js') !!}"></script>


<script src="{!! asset('js/client/main.js') !!}"></script>

<script src="{!! asset('js/client/hotel/hotel.js') !!}"></script>


<script src="{!! asset('client/js/bootstrap-datepicker.js') !!}"></script>
<script src="{!! asset('client/js/jquery-migrate-3.0.1.min.js') !!}"></script>

<script src="{!! asset('client/js/aos.js') !!}"></script>
<script src="{!! asset('client/js/jquery.timepicker.min.js') !!}"></script>




<script src="{!! asset('client/js/popper.min.js') !!}"></script>
<script src="{!! asset('client/js/jquery.stellar.min.js') !!}"></script>
<script src="{!! asset('client/js/jquery.fancybox.min.js') !!}"></script>
<script src="{!! asset('client/js/main.js') !!}"></script>


{{-- <script type='text/javascript' src='http://ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js'></script> --}}
<script src="{!! asset('unitegallery/dist/js/unitegallery.min.js') !!}"></script>
<script src="{!! asset('unitegallery/dist/themes/default/ug-theme-default.js') !!}"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/fotorama/4.6.4/fotorama.js"></script>
<script type="text/javascript">
  jQuery(document).ready(function(){ 
    jQuery("#gallery").unitegallery(); 
    // $('.ug-default-button-hidepanel').click();
      $('.ug-default-button-play').click();
  }); 


  jQuery("gallery").unitegallery({
		
		gallery_skin:"alexis",			//it's the default skin
		// slider_bullets_skin: "alexis"		//example how to change only skin for slider bullets
	
    gallery_control_thumbs_mousewheel:true
	});
</script>